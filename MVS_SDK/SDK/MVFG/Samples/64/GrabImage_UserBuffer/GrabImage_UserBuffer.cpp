...
void SaveRawImage(int nImageNo, MV_FG_BUFFER_INFO* pstImageInfo)
{
    if (pstImageInfo)
    {
        ...
        fwrite(pstImageInfo->pBuffer, 1, pstImageInfo->nFilledSizeEx, pImageFile);
        fclose(pImageFile);
    }
}
    ...
    if (pUser)
    {
        ...
        while (!g_bExit)
        {
            ...
            else
            {
                ...
                if ((NULL != stFrameInfo.pBuffer) && (0 < stFrameInfo.nFilledSizeEx) && (SAVE_IMAGE_NUM > nSaveImage))
                {
...
int main()
{
    ...
    do
    {
        ...
        nRet = MV_FG_GetPayloadSizeEx(hStream, &nPayloadSize);
        if (MV_FG_SUCCESS != nRet)
        {
        ...
        for (unsigned int i = 0; i < BUFFER_NUMBER; i++)
        {
            ...
            nRet = MV_FG_AnnounceBufferEx(hStream, pBuffer[i], nPayloadSize, NULL, &(hBuffer[i]));
            if (MV_FG_SUCCESS != nRet)
            {
