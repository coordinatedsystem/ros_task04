    ...
    while (1)
    {
        ...
        if (MV_FG_SUCCESS == nRet)
        {
            ...
            if (MV_FG_SUCCESS == nRet)
            {
                ...
                nRet = MV_FG_DisplayOneFrameEx(m_stDevHandle[nCurCameraIndex].hDevice, m_hWndDisplay[nCurCameraIndex], &stDisplayInfo, 0);
                if(MV_FG_SUCCESS != nRet)
                {
...
void MainWindow::on_bnStartGrabbing_clicked()
{
    ...
    uint64_t nImagePayloadSize = 0;
    nRet = MV_FG_GetPayloadSizeEx(m_stDevHandle[nIndex].hStream, &nImagePayloadSize);
    if (MV_FG_SUCCESS != nRet)
    {
    ...
    for (unsigned int i = 0; i < BUFFER_NUMBER; ++i)
    {
        ...
        nRet = MV_FG_AnnounceBufferEx(m_stDevHandle[nIndex].hStream, m_stDevHandle[nIndex].pBuffer[i], nImagePayloadSize, NULL, &(m_stDevHandle[nIndex].hBuffer[i]));
        if (MV_FG_SUCCESS != nRet)
        {
