...
, m_nTriggerSource(-1)
{
    ...
    m_hWnd[2] = (void *)ui->widgetDisplay_3->winId();
    m_hWnd[3] = (void *)ui->widgetDisplay_4->winId();
    ...
    while(m_bThreadState)
    {
        if (m_bStartGrabbing)
        {
            ...
            if (MV_FG_SUCCESS == nRet)
            {
                ...
                {
                    // 如果图像大小由小变大需要重新分配缓存
                    if (stFrameInfo.nFilledSizeEx / m_nLineNum > nImageBufferSize)
                    {
                        for (unsigned int i = 0; i < m_nLineNum; i++)
                        {
                            ...
                            pImageBufferList[i] = (unsigned char*)malloc(sizeof(unsigned char) * stFrameInfo.nFilledSizeEx / m_nLineNum);
                            if (NULL != pImageBufferList[i])
                            {
                                stReconstructInfo.stOutputImageInfo[i].pImageBuf = pImageBufferList[i];
                                stReconstructInfo.stOutputImageInfo[i].nImageBufSize = stFrameInfo.nFilledSizeEx / m_nLineNum;
                            }
                            else
                            {
                        ...
                        nImageBufferSize = stFrameInfo.nFilledSizeEx / m_nLineNum;
                    }
                    ...
                    stReconstructInfo.stInputImageInfo.nImageBufLen = stFrameInfo.nFilledSizeEx;
                    stReconstructInfo.enReconstructMode = (MV_FG_RECONSTRUCT_MODE)(MV_FG_SPLIT_BY_LINE_MODE | m_nLineNum);
                    ...
                    for (unsigned int i = 0; i < m_nLineNum; i++)
                    {
                        ...
                        nRet = MV_FG_DisplayOneFrameEx(m_hStream, m_hWnd[i], &stDisplayInfo, 0);
                        if (MV_FG_SUCCESS != nRet)
                        {
                            MV_FG_ReleaseFrameBuffer(m_hStream, &stFrameInfo);
                ...
                else    // 不满足分时频闪条件
                {
                    ...
                    nRet = MV_FG_DisplayOneFrameEx(m_hStream, m_hWnd[0], &stDisplayInfo, 0);
                    if (MV_FG_SUCCESS != nRet)
                    {
                        MV_FG_ReleaseFrameBuffer(m_hStream, &stFrameInfo);
                        printf("threadFunc DisplayOneFrame faild nRet %d\n", nRet);
...
void MainWindow::FlushControlButton(bool bOpen, bool bStart)
{
    if ((!bOpen) && (!bStart))
    {
        ...
        if(m_bMultiLightControl)
        {
            ui->ComboMultiLightControl->setEnabled(false);
        }
    }
    else if ((bOpen) && (!bStart))
    {
        ...
        if(m_bMultiLightControl)
        {
            ui->ComboMultiLightControl->setEnabled(true);
        }
    }
    else if ((bOpen) && (bStart))
    {
...
void MainWindow::on_bnCamOpen_clicked()
{
    ...
    nRet = GetMultiLightControlNum();
    if (MV_FG_SUCCESS != nRet)
    {
        m_bMultiLightControl = false;
        ShowErrorMsg("Get MultiLightControl Num Failed", nRet);
    }
    else
    {
        m_bMultiLightControl = true;
    }
...
void MainWindow::on_bnStartGrabbing_clicked()
{
    ...
    if (TRIGGER_MODE_ON ==  m_nTriggerMode && ui->CheckSoftTrigger->isChecked())
    {
...
int MainWindow::SetTriggerSource()
{
    if (ui->CheckSoftTrigger->isChecked())
    {
        ...
        if (TRIGGER_MODE_ON ==  m_nTriggerMode && true == m_bStartGrabbing)
        {
            ui->bnSoftTriggerOnce->setEnabled(true);
        }
    }
...
int MainWindow::GetMultiLightControlNum()
{
    int nRet = 0;
    MV_FG_ENUMVALUE stEnumValue = { 0 };
    // ch:获取 MultiLightControl | en:Get MultiLightControl
    ui->ComboMultiLightControl->clear();
    ...
    if (MV_FG_SUCCESS == nRet)
    {
        ...
        {
            ui->ComboMultiLightControl->addItem(stEnumValue.strSymbolic[i]);
        ...
        ui->ComboMultiLightControl->setCurrentIndex(nIndex);
...
void MainWindow::on_RadioTrigger_clicked()
{
    ...
    if (true == m_bStartGrabbing && ui->CheckSoftTrigger->isChecked())
    {
        ui->bnSoftTriggerOnce->setEnabled(true);
    }
    else
...
void MainWindow::on_ComboMultiLightControl_currentIndexChanged(const QString &arg1)
{
    if (m_bMultiLightControl)
    {
        int nRet = MV_FG_SUCCESS;
        if(!arg1.isEmpty())
        {
            nRet = MV_FG_SetEnumValueByString(m_hDevice, "MultiLightControl", arg1.toStdString().c_str());
            if (MV_FG_SUCCESS != nRet)
            {
                 ShowErrorMsg("Set MultiLightControl Num failed.", nRet);
            }
        }
