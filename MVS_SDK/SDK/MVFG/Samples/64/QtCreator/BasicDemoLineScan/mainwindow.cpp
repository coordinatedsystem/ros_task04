...
void MainWindow::ThreadFun()
{
    ...
    MV_FG_DISPLAY_FRAME_INFO_EX stDisplayInfo = { 0 };  // ch:显示的图像信息 | en:Display frame info
    ...
    while(m_bThreadState)
    {
        if (m_bStartGrabbing)
        {
            ...
            {
                printf("FrameNum:%ld Width:%d Height:%d, nImageBufLen:%lld, enPixelType:%x\n", stFrameInfo.nFrameID, stFrameInfo.nWidth, 
                        stFrameInfo.nHeight, stFrameInfo.nFilledSizeEx, stFrameInfo.enPixelType);
                fflush(stdout);

                // 用于保存图片
                pthread_mutex_lock(&m_hSaveImageMux);
                memset(&m_stImageInfo, 0, sizeof(MV_FG_INPUT_IMAGE_INFO_EX));
                if (NULL == m_pDataBuf || stFrameInfo.nFilledSizeEx > m_nDataBufSize)
                {
                    if (m_pDataBuf)
                    {
                        free(m_pDataBuf);
                    ...
                    m_pDataBuf = (unsigned char*)malloc(sizeof(unsigned char) * stFrameInfo.nFilledSizeEx);
                    if (NULL == m_pDataBuf)
                    {
                        pthread_mutex_unlock(&m_hSaveImageMux);
                        printf("thread no Memory...\n");
                    }
                    m_nDataBufSize = stFrameInfo.nFilledSizeEx;
                }
                memset(m_pDataBuf, 0, m_nDataBufSize);
                memcpy(m_pDataBuf, stFrameInfo.pBuffer, stFrameInfo.nFilledSizeEx);
                ...
                m_stImageInfo.nImageBufLen  = stFrameInfo.nFilledSizeEx;
                pthread_mutex_unlock(&m_hSaveImageMux);
                ...
                // 配置显示图像的参数
                memset(&stDisplayInfo, 0, sizeof(MV_FG_DISPLAY_FRAME_INFO_EX));
                ...
                stDisplayInfo.nImageBufLen  = stFrameInfo.nFilledSizeEx;
                nRet = MV_FG_DisplayOneFrameEx(m_hStream, m_hWnd, &stDisplayInfo, 0);
                if (MV_FG_SUCCESS != nRet)
                {
                    MV_FG_ReleaseFrameBuffer(m_hStream, &stFrameInfo);
...
void MainWindow::on_bnIntEnum_clicked()
{
    ...
    bool bChanged = false;
    nRet = MV_FG_UpdateInterfaceList(MV_FG_CAMERALINK_INTERFACE | MV_FG_CXP_INTERFACE | MV_FG_XoF_INTERFACE, &bChanged);
    if (MV_FG_SUCCESS != nRet)
    {
...
int MainWindow::SaveImage(SAVE_IAMGE_TYPE enSaveImageType)
{
    ...
    int             nRet = 0;
    uint64_t        nMaxImageLen = (uint64_t)m_stImageInfo.nWidth * m_stImageInfo.nHeight * 4 + 2048; 
    time_t          rawtime;
    struct tm*      timeinfo;
    
    if ((Image_Bmp == enSaveImageType || Image_Jpeg == enSaveImageType))
    {
        if (m_stImageInfo.nImageBufLen > 0xFFFFFFFF)
        {
            pthread_mutex_unlock(&m_hSaveImageMux);
            return MV_FG_ERR_NOT_IMPLEMENTED;
        }
        else
        {
    ...
    do
    {
        if (Image_Bmp == enSaveImageType)
        {
            ...
            stBmpInfo.stInputImageInfo.nWidth   = m_stImageInfo.nWidth;
            stBmpInfo.stInputImageInfo.nHeight   = m_stImageInfo.nHeight;
            stBmpInfo.stInputImageInfo.enPixelType   = m_stImageInfo.enPixelType;
            stBmpInfo.stInputImageInfo.pImageBuf   = m_stImageInfo.pImageBuf;
            stBmpInfo.stInputImageInfo.nImageBufLen   = m_stImageInfo.nImageBufLen;
        ...
        else if (Image_Jpeg == enSaveImageType)
        {
            ...
            stJpgInfo.stInputImageInfo.nWidth   = m_stImageInfo.nWidth;
            stJpgInfo.stInputImageInfo.nHeight   = m_stImageInfo.nHeight;
            stJpgInfo.stInputImageInfo.enPixelType   = m_stImageInfo.enPixelType;
            stJpgInfo.stInputImageInfo.pImageBuf   = m_stImageInfo.pImageBuf;
            stJpgInfo.stInputImageInfo.nImageBufLen   = m_stImageInfo.nImageBufLen;
        ...
        {
            MV_FG_SAVE_TIFF_TO_FILE_INFO_EX stTiffInfo ={0};
            ...
            sprintf(szFileName, "Image_w%d_h%d_%02d%02d%04d.tif", 
                m_stImageInfo.nWidth, m_stImageInfo.nHeight, timeinfo->tm_mday, timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec);
            stTiffInfo.pcImagePath = (char*)szFileName;

            // ch:保存Tiff图像 | en:Save to Tiff
            nRet = MV_FG_SaveTiffToFileEx(m_hStream,&stTiffInfo);
            if (MV_FG_SUCCESS != nRet)
            {
                 break;
            }
        ...
        {
            MV_FG_SAVE_PNG_TO_FILE_INFO_EX stPngInfo = {0};
            
            stPngInfo.stInputImageInfo = m_stImageInfo;
            stPngInfo.nPngCompression  = 6;
            stPngInfo.enCfaMethod      = MV_FG_CFA_METHOD_OPTIMAL;
            sprintf(szFileName, "Image_w%d_h%d_%02d%02d%04d.png", 
                m_stImageInfo.nWidth, m_stImageInfo.nHeight, timeinfo->tm_mday, timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec);
            stPngInfo.pcImagePath = (char*)szFileName;

            // ch:保存Png图像 | en:Save to Png
            nRet = MV_FG_SavePngToFileEx(m_hStream,&stPngInfo);
            if (MV_FG_SUCCESS != nRet)
            {
                break;
            }
...
void MainWindow::EnableIFParamsControls()
{
    ...
    ui->comboLPolarity->setEnabled(m_bOpenIF && !m_bStartGrabbing && m_bLineInputPolarity);
    ...
    ui->comboCCSel->setEnabled(m_bOpenIF && !m_bStartGrabbing && m_bCCSelector);
    ui->comboCCSource->setEnabled(m_bOpenIF && !m_bStartGrabbing && m_bCCSource);
