...
void MainWindow::ThreadFun(int nCurListIndex, int nCurCameraIndex)
{
    ...
    MV_FG_DISPLAY_FRAME_INFO_EX stDisplayFrameInfo = {0};
    ...
    while(m_bStartGrabbing && m_bDeviceGrabbingFlag[nCurListIndex][nCurCameraIndex]) 
    {
        ...
        if (NULL != stFrameInfo.pBuffer)
        {
            ...
            stDisplayFrameInfo.nImageBufLen = stFrameInfo.nFilledSizeEx;
            ...
            nRet =  MV_FG_DisplayOneFrameEx(m_stInterface[nCurListIndex].hStream[nCurCameraIndex], 
                        m_hWndDisplay[nCurListIndex == 0 ? nCurCameraIndex : nCurCameraIndex + 4], &stDisplayFrameInfo, 0);
            if (MV_FG_SUCCESS != nRet)
            {
